...
export default function ReportModal({ isOpen, onClose, result, onTopUp }: ReportModalProps) {
    ...
    const handleDownloadPDF = () => {
        ...
        doc.setFontSize(14);
        doc.setTextColor(0, 0, 0);
        doc.text(`TARGET: ${result.pairData.baseToken.name} (${result.pairData.baseToken.symbol})`, margin, y);
        y += 8;

        doc.setFontSize(10);
        doc.setFont("courier", "normal");
        doc.text(`CONTRACT: ${result.pairData.baseToken.address}`, margin, y);
        y += 15;
        ...
        // Save
        const filename = `NEXUS_REPORT_${result.pairData.baseToken.symbol}_${new Date().toISOString().slice(0, 10)}.pdf`;
        doc.save(filename);
    };
        ...
        <AnimatePresence>
            {isOpen && (
                <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
                    ...
                    >
                        {/* Header with ID */}
                        <div className="p-6 border-b border-gold-primary/20 flex flex-col md:flex-row justify-between items-start md:items-center bg-gradient-to-r from-gold-primary/10 to-transparent gap-4">
                            <div>
                                ...
                                <div className="space-y-1 mt-2">
                                    ...
                                    <div className="flex items-center gap-2 text-[10px] font-mono text-gray-500 bg-black/40 px-2 py-1 rounded w-fit border border-white/5">
                                        <Lock size={10} />
                                        <span>CONTRACT:</span>
                                        <span className="text-gold-primary select-all">{pair?.baseToken?.address || "UNKNOWN"}</span>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div className="flex flex-wrap items-center gap-3">
                                ...
                                <button
                                    ...
                                    className="px-4 py-2 rounded-full font-bold text-xs flex items-center gap-2 transition-all bg-gold-primary text-black hover:bg-white hover:scale-105 shadow-[0_0_20px_rgba(212,175,55,0.4)]"
                                >
                                    <FileWarning size={16} />
                                    DOWNLOAD PDF (1 CR)
                                </button>
