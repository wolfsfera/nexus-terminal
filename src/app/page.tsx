...
export default function NexusPage() {
  ...
  const runAnalysis = async () => {
    ...
    // CREDIT CHECK
    const { spendCredits } = useCredits(); // Make sure to destructure this inside component body, not here.
    // Wait, hooks must be top level. I need to modify the hook usage in lines 22-34 first.
    // Let's refactor the whole function block properly.

    // ERROR in thought: I cannot useHook inside a regular function.
    // I need to destructure `spendCredits` at the top level of the component (line 25 approx).

    // RE-READING FILE check line 23...
    // Line 23: const { language, setLanguage, t } = useLanguage();
    // I need to add useCredits there.

    // Changing approach to multi_replace to add hook import and usage correctly.
    return;
  };
  setResult(null);
